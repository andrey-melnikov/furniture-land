...
namespace FactoryFramework
{
    public class Conveyor : LogisticComponent, IInput, IOutput
    {
        ...
        public void UpdateMesh(bool finalize = false, Collider[] ignored = null, int startskip = 0, int endskip = 0)
        {
            ...
            bool collision = PathFactory.CollisionAlongPath(p, 0.5f, 0.25f, ~0, ignored, startskip: startskip, endskip: endskip); //only collide belt collideable layer
            if (collision)
            {
                _validMesh = false;
            }

            frameFilter.mesh = BeltMeshGenerator.Generate(p, frameBM, length, .35f);
            beltFilter.mesh = BeltMeshGenerator.Generate(p, beltBM, length, .35f, 1f, true);
